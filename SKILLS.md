...
- The postmortem engine uses a configurable LLM (Anthropic, OpenAI, or Ollama) for report generation. Default is Ollama (`localhost:11434`) so it works locally without API keys. If no LLM is reachable, it produces a structured-but-unnarrated fallback report.
